...
exports.saveScene = async (req, res) => {
    try {
        ...
        // 保存只更新已有场景，不能将已删除的场景重新创建。
        if (!await Scene.exists({ sceneId })) {
            return res.status(404).json({
                success: false,
                code: 'SCENE_NOT_FOUND',
                message: '场景已删除或不存在，无法保存'
            });
        }
        ...
        if (metadata.lightingConfig !== undefined) sceneData.lightingConfig = metadata.lightingConfig;
        ...
        // 检查期间也可能发生删除，更新失败时清理本次写入的孤立对象。
        if (!savedScene) {
            await SceneObject.deleteMany({ sceneId });
            return res.status(404).json({
                success: false,
                code: 'SCENE_NOT_FOUND',
                message: '场景已删除或不存在，无法保存'
