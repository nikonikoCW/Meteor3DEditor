...
/**
 * 修改 3D Tiles 或高斯泼溅资产绑定的外部 URL。
 * @param {string} id - 资产 ID
 * @param {string} url - 新的外部 URL
 * @returns {Promise<Object>}
 */
export async function updateRegisteredAssetUrl(id, url) {
    try {
        const response = await fetch(`${API_BASE_URL}/assets/${id}/registered-url`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (!response.ok || !data.success) {
            throw new Error(data.message || '修改绑定 URL 失败');
        }
        return data;
    } catch (error) {
...
export function uploadGaussianSplat(file, name, onProgress) {
    return new Promise((resolve, reject) => {
        const request = new XMLHttpRequest();
        request.open('POST', `${API_BASE_URL}/assets/upload-gaussian-splat`);
        ...
        request.onload = () => {
            try {
                ...
                if (request.status < 200 || request.status >= 300) throw new Error(result.message || '上传失败');
                resolve(result);
