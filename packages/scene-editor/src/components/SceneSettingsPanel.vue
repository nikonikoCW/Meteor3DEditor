  ...
  <div class="scene-settings-panel">
    ...
    <div class="section">
      ...
      <div class="prop-row switch-row">
        <label for="manual-lighting-enabled">手动光照</label>
        <div class="switch-container">
          <label class="switch">
            <input id="manual-lighting-enabled" type="checkbox" v-model="lightingConfig.enabled"
              :disabled="!lightingReady" @change="onLightingChange">
            <span class="slider lighting-switch"></span>
          </label>
          <span class="switch-label">{{ lightingConfig.enabled ? '开启' : '关闭' }}</span>
        </div>
      </div>
      <div v-if="lightingConfig.enabled">
      <label v-for="control in LIGHTING_CONTROLS" :key="control.key" class="lighting-control">
        <span>{{ control.label }}</span>
        <output class="panel-range-value">{{ lightingConfig[control.key].toFixed(control.step === 1 ? 0 : 1) }}</output>
        <input type="range" v-model.number="lightingConfig[control.key]"
          :min="control.min" :max="control.max" :step="control.step"
          :disabled="!lightingReady" @input="onLightingChange">
      </label>
      <div class="lighting-actions">
        <button :disabled="!lightingReady" @click="applyLighting(REFERENCE_LIGHTING)">应用参考光照</button>
        <button :disabled="!lightingReady" @click="applyLighting(DEFAULT_LIGHTING)">重置光照</button>
      </div>
      <div class="hint">拖动即时生效，点击保存场景后保留。重置将灯光强度归零、曝光恢复 1.0，保持总开关开启。</div>
      </div>
      <div v-else class="hint">手动光照已关闭，曝光恢复 1.0，环境贴图不受影响。开启后可调整参数。</div>
    </div>

    <div class="section">
      <h4>初始视角</h4>
      <div class="hint view-hint">
        保存场景初始视角，并将当前画面更新为场景封面
      </div>
      <button
        class="save-view-btn"
        :disabled="isSavingInitialView"
        @click="onSaveInitialView"
      >
        {{ isSavingInitialView ? '保存中...' : '保存视角并更新封面' }}
      </button>
      <div class="view-status">
        {{ editorStore.sceneMetadata.initialView ? '已设置初始视角' : '尚未设置初始视角' }}
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="bvh-depth-control" v-if="showBVH">
        <div class="depth-row">
          ...
          <span class="depth-value panel-range-value">{{ bvhDepth }}</span>
        </div>
        <input type="range" aria-label="BVH 显示深度" v-model.number="bvhDepth" @input="onBVHDepthChange" min="1" max="20" step="1">
...
import { saveInitialView, uploadSceneThumbnail } from '../services/sceneService';
import { message } from '../utils/message';
import { LIGHTING_CONTROLS, DEFAULT_LIGHTING, REFERENCE_LIGHTING } from '@meteor3d/core/src/LightingManager.js';

const editorStore = useEditorStore();
const isSavingInitialView = ref(false);
...
const lightingConfig = reactive({ ...DEFAULT_LIGHTING });
const lightingReady = ref(false);
const syncLighting = () => {
  const manager = window.editor?.sceneManager?.lightingManager;
  lightingReady.value = !!manager;
  if (manager) Object.assign(lightingConfig, manager.getConfig());
};
const onLightingChange = () => {
  window.editor?.sceneManager?.lightingManager?.setConfig(lightingConfig);
};
const applyLighting = (config) => {
  // 参数预设只更新滑块值，总开关由用户独立控制。
  for (const { key } of LIGHTING_CONTROLS) {
    lightingConfig[key] = config[key];
  }
  onLightingChange();
};
...
const captureCurrentView = (sceneManager) => {
  const renderer = sceneManager.renderer;
  const canvas = renderer?.domElement;
  if (!renderer || !canvas) {
    return Promise.reject(new Error('渲染画布不可用'));
  }

  // WebGL 未开启 preserveDrawingBuffer，因此在导出前同步渲染当前视角。
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        reject(new Error('截图内容为空'));
      }
    }, 'image/png');
  });
};

const onSaveInitialView = async () => {
  const sceneManager = window.editor?.sceneManager;
  const sceneId = window.editor?.persistenceManager?.currentSceneId;

  if (!sceneManager || !sceneId) {
    message.error('编辑器尚未初始化');
    return;
  }

  isSavingInitialView.value = true;
  const currentView = sceneManager.getView();
  let thumbnailBlob = null;
  let screenshotError = null;

  try {
    try {
      thumbnailBlob = await captureCurrentView(sceneManager);
    } catch (error) {
      screenshotError = error;
      console.error('当前视角截图失败:', error);
    }

    const savedView = await saveInitialView(sceneId, currentView);
    editorStore.setSceneMetadata({ initialView: savedView });

    if (!thumbnailBlob) {
      message.warning(`初始视角已保存，但封面截图失败：${screenshotError.message}`);
      return;
    }

    try {
      const thumbnail = await uploadSceneThumbnail(sceneId, thumbnailBlob);
      editorStore.setSceneMetadata({ thumbnail });
      message.success('初始视角和场景封面已保存');
    } catch (error) {
      message.warning(`初始视角已保存，但场景封面保存失败：${error.message}`);
    }
  } catch (error) {
    message.error(`初始视角保存失败：${error.message}`);
  } finally {
    isSavingInitialView.value = false;
  }
};

onMounted(() => {
  window.addEventListener('editor-ready', syncLighting);
  syncLighting();
  if (window.editor && window.editor.sceneManager) {
    // 首次如果不存在值，则初始化一次
...
onBeforeUnmount(() => {
  window.removeEventListener('editor-ready', syncLighting);
  if (window.editor && window.editor.sceneManager) {
...
.lighting-control input { grid-column: 1 / -1; }
...
.switch input:checked + .lighting-switch {
  background-color: #0066cc;
}

input:focus-visible + .lighting-switch {
  outline: 2px solid #8bbcf0;
  outline-offset: 2px;
}

input:disabled + .lighting-switch {
  opacity: 0.4;
  cursor: not-allowed;
}

.lighting-actions button {
  padding: 6px 8px;
  background: #29476b;
...
.lighting-actions button:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
