import { defineStore } from 'pinia';
import { ref, markRaw } from 'vue';

export const useEditorStore = defineStore('editor', () => {
    const selectedObject = ref(null);
    const sceneObjects = ref([]); // For tree view
    const ignoreInvisibleOnPick = ref(false);

    // 场景元数据
    const sceneMetadata = ref({
        name: '未命名场景',
        description: '',
        cameraFar: 1000000,
        initialView: null,
        isFeatured: false
    });

    // 精品案例场景解锁状态
    const isUnlocked = ref(false);

    // 用户明确接受资源缺失后，允许编辑器保存当前已加载的场景快照。
    // 该状态属于编辑器交互策略，不改变 Core 的 isReady 语义。
    const allowPartialSave = ref(false);

    // 版本号：markRaw 对象的深层属性变化无法被 Vue 追踪，
    // 通过递增此值来通知依赖组件（如 SceneTree）强制刷新。
    const treeVersion = ref(0);

    function setSceneMetadata(metadata) {
        sceneMetadata.value = { ...sceneMetadata.value, ...metadata };
        // 每次设置场景元数据时重置解锁状态
        isUnlocked.value = false;
    }

    function allowPartialSceneSave() {
        allowPartialSave.value = true;
    }

    function resetPartialSavePermission() {
        allowPartialSave.value = false;
    }

    function selectObject(object) {
        selectedObject.value = object ? markRaw(object) : null;
    }

    function clearSelection() {
        selectedObject.value = null;
    }

    function addObject(object) {
        sceneObjects.value.push(markRaw(object));
    }

    function removeObject(object) {
        const index = sceneObjects.value.indexOf(object);
        if (index > -1) {
            sceneObjects.value.splice(index, 1);
        }
    }

    function resetObjects(objects = []) {
        // Replace with fresh array to avoid residue across scene loads
        sceneObjects.value = objects.map(obj => markRaw(obj));
    }

    /** 通知场景树等组件刷新（用于 markRaw 对象属性变更后） */
    function notifyTreeUpdate() {
        treeVersion.value++;
    }

    return {
        selectedObject,
        sceneObjects,
        ignoreInvisibleOnPick,
        sceneMetadata,
        isUnlocked,
        allowPartialSave,
        treeVersion,
        setSceneMetadata,
        allowPartialSceneSave,
        resetPartialSavePermission,
        selectObject,
        clearSelection,
        addObject,
        removeObject,
        resetObjects,
        notifyTreeUpdate
    };
});
