...
export class TransformManager {
    constructor(sceneManager, historyManager, persistenceManager = null) {
        ...
        // TransformControls 单独在覆盖层渲染，避免参与 OutlinePass 后处理。
        this.helper = this.controls.getHelper();
        sceneManager.addOverlayObject(this.helper);
    }
    ...
    attach(object) {
        ...
        if (object?.userData?.modelType === 'GaussianSplat' && this.mode === 'scale') {
    ...
    /**
     * 设置变换模式
     * @param {string} mode - 'select', 'translate', 'rotate', 或 'scale'
     */
    setMode(mode) {
    ...
    dispose() {
        this.controls.detach();
        this.sceneManager?.removeOverlayObject(this.helper);
